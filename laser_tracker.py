...
class LaserTracker(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        # except:
        #     pass
        self.neural_state = x#self.neural_state*self.factor + x
            ...
            with Durin('durin0.local') as durin:
                try:
                    while True:
                        ...
                        #     durin(Move(0,0,0))
                        #     break
                        if frame_viz[indices[0], indices[1], 1] > 50:
                            if indices[1] < 180:
                                continue
                            ind_x = indices[1] * 2 - 410
                            complete_motion_r.append(np.clip(-1*(indices[0]-320), -200, 200))
                            complete_motion_x.append(np.clip(3.5*2*(ind_x-410), -200, 200))
                            counter_xy += 1
                            if counter_xy >= 15:
                    ...
                    # np.save('test.npy', np.stack(tof_data))
                    # np.save('StillIncomingFootBall_frames.npy', np.stack(total_frames))
                    # np.save('StillIncomingFootBall_timestamps.npy', np.array(timestamps))
                    # print("Saved files!")
                    pass
